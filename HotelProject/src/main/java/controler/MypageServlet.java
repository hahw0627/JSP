...
public class MypageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String currentUserName = "방문자"; // JSP에 표시될 사용자 이름의 기본값

        // 실제 로그인 구현 시 세션에서 사용자 정보를 가져옵니다.
        if (session != null && session.getAttribute("loggedInUser") != null) {
            UserDTO loggedInUser = (UserDTO) session.getAttribute("loggedInUser");
            currentUserId = loggedInUser.getUserId();
            currentUserName = loggedInUser.getName(); // 실제 로그인된 사용자의 이름 사용
        } else {
            // 로그인되지 않은 경우 (또는 테스트 목적으로) 임시 사용자 정보 사용
            // 실제 운영 시에는 로그인 페이지로 리다이렉트하는 것이 일반적입니다.
            currentUserId = "testUser"; // 임시 사용자 ID
            currentUserName = "testUser (임시)"; // 임시 사용자 이름
            // response.sendRedirect(request.getContextPath() + "/login.jsp"); // 실제 구현 시 주석 해제
            // return; // 리다이렉트 후에는 더 이상 서블릿 코드를 진행하지 않도록 return
        }

        // 람다에서 사용할 수 있도록 currentUserId 값을 사실상 final인 새 변수에 할당합니다.
        final String userIdForFilter = currentUserId;

        List<ReservationDTO> userReservations = getAllMockReservations().stream()
                                                .filter(r -> userIdForFilter.equals(r.getUserId()))
                                                .collect(Collectors.toList());

        request.setAttribute("reservationList", userReservations);
        request.setAttribute("userName", currentUserName); // JSP에서 사용자 이름 표시용 (userId 대신 name 사용)
